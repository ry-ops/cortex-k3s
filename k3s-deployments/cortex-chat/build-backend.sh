#!/bin/bash
# Build and deploy cortex-chat-backend-simple with conversation fixes
set -e

echo "=== Cortex Chat Backend Build and Deploy ==="
...
spec:
  ...
  - name: source
    hostPath:
      path: /Users/ryandahlberg/Projects/cortex/k3s-deployments/cortex-chat/backend-simple
      type: Directory
  containers:
  - name: copy
    image: busybox
...
kubectl wait --for=condition=Ready pod/backend-copy-context -n $NAMESPACE --timeout=60s || true
sleep 5
kubectl wait --for=jsonpath='{.status.phase}'=Succeeded pod/backend-copy-context -n $NAMESPACE --timeout=120s
