...
// Initialize global token throttle (shared across all services)
const tokenThrottle = new TokenThrottle(redisClient);
...
async function callClaude(messages, tools, sseWriter = null, retryCount = 0, systemPrompt = null) {
  ...
  // Apply global token throttle BEFORE making API call
  const throttleResult = await tokenThrottle.throttle(messages, tools, systemPrompt);
  if (throttleResult.waitMs > 0) {
    metrics.recordThrottleWait(throttleResult.waitMs / 1000, throttleResult.currentUsage);
    if (sseWriter) {
      sseWriter(JSON.stringify({
        type: "throttle_wait",
        message: `Waiting for rate limit (${throttleResult.currentUsage}/28000 tokens used)`
      }));
    }
  }
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        try {
          ...
          // Record successful request with usage metrics
          const durationSeconds = (Date.now() - requestStartTime) / 1000;
